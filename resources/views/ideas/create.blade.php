<x-layout>
    <div class="max-w-2xl mx-auto">
        <h1 class="text-2xl font-bold mb-6">Create New Idea</h1>
        ...
        <form method="POST" action="{{ route('ideas.store') }}" class="space-y-4">
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="5"
                          class="w-full rounded-lg border border-gray-300 px-4 py-2 focus:ring-2 focus:ring-blue-500">{{ old('description') }}</textarea>
                @error('description')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
